...
function recipeUtils.makeRecipe(attributes)
    -- if not attributes.expensive then
        -- attributes.expensive = {
            ...
            -- results = attributes.results or {{type = "item", name = attributes.result, amount = 1}},
        ...
        -- end
    -- end
	-- if attributes.name == "nuclear-generator-rampant-arsenal" then
		-- error(attributes.name..":"..serpent.dump(attributes.ingredients))
	-- end
    data:extend({
            {
                type = "recipe",
